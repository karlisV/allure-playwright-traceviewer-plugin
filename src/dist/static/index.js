...
const TraceView = Backbone.Marionette.View.extend({
  ...
  fetchTraceContent: async function () {
    ...
    if (att && att.type.includes("text")) {
      const attachmentLink = `${this.getBasePath()}data/attachments/${
        att.source
      }`;
      try {
  ...
  render: function () {
    ...
    const iframe = this.$el.find("#trace-iframe");

    if (iframe.length) {
      iframe.attr("src", `${TRACE_VIEW_APP_URL}/?trace=${this.traceContent}`);
